...
if DEBUG >0: print(type(train_set_x_orig))

import os
import ftplib
def upload(filename, dest_folder = '/www/public/'):
    session = ftplib.FTP('ftp.pollisrl.it','pollisrl.it','GIO1301')
...
# ftp_libWe added "_orig" at the end of image datasets (train and test) because we are going to preprocess them. After preprocessing, we will end up with train_set_x and test_set_x (the labels train_set_y and test_set_y don't need any preprocessing).
# 
...
def propagate(w, b, X, Y):
    ...
    #cost = -1/m*np.sum(Y.dot(np.log(np.transpose(A)) + (1 - Y).dot(np.log(1 - np.transpose(A)))))  ### ***WRONG!!!***
...
def model(X_train, Y_train, X_test, Y_test, num_iterations = 2000, learning_rate = 0.5, print_cost = False):
    ...
    m = X_train.shape[0]
    w, b = initialize_with_zeros(dim = m)
